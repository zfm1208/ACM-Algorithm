...
/*
    正向模拟的思路：我要选那几行进行翻转....?
    不对！，动一行所有列都会改变，没办法做到全局最优

    所以我们要跳出思维，逆向思考，如果这一列要合法，需要动哪几行？
    然后使用map对这几行进行一个 + 1 ,为了方便我们可以进行哈希处理
    ...
    Final：
    操作是作用到行上面的，但影响的所有列，得分看的是列（每一行单独判断合不合法）
    求最大满足数量
    操作： xor
    对行进行操作，会影响多有列
    但考虑列的话，列与列直接是互相独立的。
    ...
    然后对这个变化情况进行投票处理，看看你哪种方案带来的收益最多
    然后灵活进行XOR操作，简化代码
    X xor Y = C
    X xor C = Y
    C xor Y = X
    是等价的：(x xor y xor y = x)
*/
